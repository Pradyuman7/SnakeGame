...
;; I am, at TU Delft, making snake game in assembly. 
;; This is pretty much it, for those who read this, Thank you and sorry for boring you.
;; Now, to the game...
;;
;; To build:
;;	nasm -f bin -o projectSnake projectSnake.asm && chmod +x projectSnake
;;
...
;;
;; For Graphics of the snake Video terminal(VT100) has been used(to get snake properly)
;; and it may or may not support the simple linux terminal. Also, I know VT200 came in 1980s, but I
;; still used VT100. Why? I don't know for sure, myself :)
;;
;; I learnt using this simple "Graphics"(or should I say characters) of snake length growing 
;; and snake by reading the docs from vt100.net
;;
;; If you're playing on a Linux console and you see a
;; bunch of lowercase letters instead of lines, try typing "echo -e '\e)0'" before starting the game. 
;; If that doesn't help, or if instead you see jumbled non-ASCII characters, you may need to
;; switch to a console font with the full VT100 characters set in order to play.

BITS 32

;; macros for getting and using addresses in .text and .bss via ebp(to store the old value of base pointer).

%define	data(a)		byte ebp + ((a) - sc)

;; Sytem calls I used
;; I know, I commented very nicely, Don't judge me. I didn't want to mess up my first assembly game, after 2 weeks

%define SC_sel      142   ; eax here is sel(ebx,ecx,edx,esi,edi)
%define SC_ex       1     ; simply exit by ex(bl)
%define SC_write    4     ; eax here is werite(ebx,ecx,edx)
%define SC_read     3     ; eax here is read(ebx,ecx,edx)
%define SC_iocntrl  54    ; eax here is iocntrl(ebx,ecx,edx)
%define SC_pau      29    ; eax is paused pau()
%define SC_sTmr     104   ; eax here is sTmr(ebx,ecx,edx)
%define SC_tDay     78    ; eax is tDay(ebx,ecx)
%define SC_san      67    ; eax is san(ebx,ecx,edx)


;; iocntrl values, used input-output controls
...
;; values of signals to control the game commencement
...
;; This is how the program points directions. 
;; These values are used with OR again and again together to direct the various VT100 line-drawing characters :)
...
%define WIDTH			80
%define	HEIGHT			25
%define	CENTER			((HEIGHT - 1) / 2) * WIDTH + (WIDTH - 1) / 2
%define	RCOL			'81'
%define	BROW			'25'

;; If take more turns, decay starts, I loved the modern physics part during my JEE studies, its after-effct :)
...
;; The headers for command

		org	0x08048000 ;; I read about org on dewkumar.blogspot.com
...
;; getkey only reads a single character
;;
;; input:
;; ecx = pointer to memory byte for storing
;;
;; output:
;; al has the address of ecx
;; [ecx] = the character input or 'r' nothing found
;;
;; eax, ebx, and edx are used differently

getkey:

;; select is called and then  waiting on the first file descriptor only, and setting
;; the time to wait to zero. If select returns an error, return 'r' to
;; the caller.
;; If nothing, call read.
		...
		int	0x80       ; wondering about this? System call command. I know, I learnt assembly a lot back.
		dec	eax
		popa               ; this is pop all registers command, there's also a pusha
		jl	.return
		js	.retquit
		mov	al, SC_read
		inc	edx
...
zmove:
		...
		bswap	eax
		add	eax, esc | ('[00' << 8)
		stosd                             ; stores a dword
		pop	eax
		mov	al, ah
		inc	eax
		aam
		bswap	eax                       ; byte swap
		shr	eax, 8                    ; shift bits to right by 8
		add	eax, ';00H'
		stosd
		ret
...
;; ebp is set to point
		...
		lea	edx, [data(termios)]
		mov	ecx, get
		xor	ebx, ebx
		lea	eax, [byte ebx + SC_ioctl]
		int	0x80
		mov	eax, [data(termios.lflag)]
		push	eax
		and	eax, byte ~(ic | echo)
		mov	[data(termios.lflag)], eax
		inc	ecx
		lea	eax, [byte ebx + SC_ioctl]
		int	0x80
		pop	dword [data(termios.lflag)]
...
;; food block on the first iteration of the main loop.
		...
		db	0xA3
		dd	snake
		mov	byte [data(scr) + eax], E | W
		mov	[data(foodpos)], eax
...
;; everything on the first iteration of the main loop.
		...
		jp	.gowest
		mov	al, E
.gowest:	mov	[data(dir)], al
		mov	byte [data(key + 1)], FF
...
;; If a 'r' was retrieved, exit the program at once. If a Ctrl-L was
...
;; advance the tail pointer.
		...
		mov	esi, [data(ringhead)]
		mov	ebx, [data(ringtail)]
		dec	dword [data(growing)]
		jns	.growing
		inc	dword [data(growing)]
		mov	eax, [snake + ebx*4]
		mov	dl, 0
		call	addvtchar
		inc	byte [data(ringtail)]
.growing:
...
;; location.
		...
		mov	eax, [data(foodpos)]
		sub	eax, esi
		jns	.positive
		neg	eax
.positive:	dec	eax
		jle	.eating
		dec	byte [data(foodlife)]
		jnz	.endfoodrelay
		dec	byte [data(foodvalue)]
		jz	.newfood
		mov	byte [data(foodlife)], F_LIFE
		mov	eax, [data(foodpos)]
		call	zmove
		mov	eax, esc | ('[7m' << 8)
		stosd
		mov	eax, '0 ' | (esc << 16) | ('[' << 24)
		add	al, [data(foodvalue)]
		stosd
		mov	eax, '27m' | (SI << 24)
		stosd
...
.eating:	mov	ecx, [data(foodvalue)]
		add	[data(growing)], ecx
		shl	dword [data(growing)], 1
		add	[data(score)], ecx
.drawscore:	mov	eax, [data(score)]
		mov	bl, 10
		mov	edx, '[:;' | ((RCOL & 0x00FF) << 24)
.updscoreloop:	div	bl
		push	eax
		mov	al, esc
		stosb
		...
		stosd
		xchg	eax, edx
		mov	al, (RCOL & 0xFF00) >> 8
		stosb
		add	ax, ('H' - ((RCOL & 0xFF00) >> 8)) | ('0' << 8)
		stosw
		pop	eax
		mov	ah, 0
		or	eax, eax
...
.newfood:	mov	eax, [data(foodpos)]
		mov	ecx, [data(foodvalue)]
		cmp	byte [data(key)], FF
		jz	.drawfood
...
.spaceforrent:	push	ebx
		fld	dword [data(seventothem31)]
		fimul	dword [esp]
		mov	eax, [data(rndseed)]
		mov	edx, 1103515245
		mul	edx
		add	eax, 12345
		shl	eax, 1
		shr	eax, 1
		mov	[data(rndseed)], eax
		fimul	dword [data(rndseed)]
		fsub	dword [data(onehalf)]
		fistp	dword [esp]
		pop	eax
...
;; head is advanced.
		...
		or	al, al
		jnz	collide
		inc	byte [data(ringhead)]
		mov	eax, [data(ringhead)]
		mov	[snake + eax*4], esi
...
;; goes to sleep until the next alarm arrives, whereupon it begins
...
;; works. Other conditions of the game produce different messages.
		...
		jz	endgame
		mov	dl, pcode
		cmp	dword [data(growing)], byte 15
		ja	endgame
		mov	dl, ecode
